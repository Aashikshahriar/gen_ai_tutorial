"""
Train a DDPM noise predictor on MNIST, noise-prediction (epsilon) variant:

    for each x0:
        repeat until convergence:
            t ~ Uniform[1, T]
            eps ~ N(0, I),  x_t = sqrt(bar_alpha_t) x0 + sqrt(1 - bar_alpha_t) eps
            take a gradient step on || eps_theta(x_t, t) - eps ||^2

After training, samples are drawn by reverse diffusion starting from
x_T ~ N(0, I): at each step the model predicts eps_hat, which is converted to
an x0_hat estimate, then the DDPM posterior update takes us down to x_0.
...
def train_one_epoch(model, schedule, loader, optimizer, device):
    ...
    for x0, _ in tqdm(loader, desc="train", leave=False):
        ...
        eps_hat = model(x_t, t)
        loss = denoising_loss(eps_hat, eps)
...
def main():
    ...
    model = NoisePredictor(in_channels=1).to(device)
    ...
    ckpt_path = os.path.join(args.out_dir, "ddpm_eps_mnist.pt")
