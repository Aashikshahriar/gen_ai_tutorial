...
Reverse step (DDPM posterior q(x_{t-1} | x_t, x0), x0 replaced by an estimate
x0_hat recovered from the denoiser's noise prediction eps_hat = eps_theta(x_t, t)
at inference time):
    x0_hat = ( x_t - sqrt(1 - bar_alpha_t) eps_hat ) / sqrt(bar_alpha_t)
...
class DiffusionSchedule:
    ...
    def x0_from_eps(self, x_t, eps_hat, t):
        bar_alpha_t = self._gather(self.bar_alphas, t)
        return (x_t - torch.sqrt(1 - bar_alpha_t) * eps_hat) / torch.sqrt(bar_alpha_t)
    ...
    def reverse_step(self, model, x_t, t_int):
        ...
        eps_hat = model(x_t, t)
        x0_hat = self.x0_from_eps(x_t, eps_hat, t)
